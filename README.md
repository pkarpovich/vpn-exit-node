...
| `TS_EXTRA_ARGS` | `--advertise-exit-node` | Extra `tailscale up` args. Keep `--advertise-exit-node` for exit-node modes; add `--advertise-tags=tag:exit-node` when registering with an OAuth client (required - see [Auth keys](#auth-keys--key-expiry)). For a self-hosted control server like [headscale](https://github.com/juanfont/headscale), add `--login-server=https://...`. |
